...
type CiscoEsaDetector struct {
	httpClient protocols.HTTPClient
}

func NewCiscoEsaDetector(httpClient protocols.HTTPClient, _ protocols.TCPClient, _ protocols.UDPClient) detectors.Detector {
	return &CiscoEsaDetector{httpClient: httpClient}
}

func (d *CiscoEsaDetector) Detect(ctx context.Context, cnProduct, vendor, target string) (string, error) {
	// 如果没有前缀协议，自动添加 https
