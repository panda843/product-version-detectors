package silver_peak_unity_edgeconnect_sd_wan_firmware
...
type SilverPeakEdgeconnectSDWANDetector struct {
	httpClient protocols.HTTPClient
}

func NewSilverPeakEdgeconnectSDWANDetector(httpClient protocols.HTTPClient, _ protocols.TCPClient, _ protocols.UDPClient) detectors.Detector {
	return &SilverPeakEdgeconnectSDWANDetector{httpClient: httpClient}
}

func (d *SilverPeakEdgeconnectSDWANDetector) Detect(ctx context.Context, cnProduct, vendor, target string) (string, error) {
	// 检查是否带有协议前缀
